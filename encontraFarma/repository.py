...
def busca_farmacias_plantao_agora():   
    ...
    farmacias = EscalaPlantao.objects.raw(
        f"""
    ...
    lista_de_farmacias = []
    for f in farmacias:
        lista_de_farmacias.append(f)

    return lista_de_farmacias


def busca_farmacias_horario_comercial_agora(): 
    ...
    farmacias = Farmacia.objects.raw(
        f"""
        SELECT * FROM encontraFarma_farmacia
